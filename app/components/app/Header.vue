...
<template>
  <header
    ...
    style="
      transition-property:
        translate, transform, background-color, color, text-decoration-color, fill, stroke;
      transition-timing-function: cubic-bezier(0.4, 0, 0.2, 1);
      transition-duration: 300ms;
    "
  >
    <div class="relative z-20 container mx-auto flex h-full items-center justify-between bg-white">
      <motion.nav
        ...
        <ul class="hidden items-center gap-4 lg:flex xl:gap-8">
          ...
          >
            <NuxtLink
              ...
              :class="[
                active === item.label ? `text-primary` : 'hover:text-primary text-black',
          ...
          <ul class="flex flex-col gap-6">
            <li v-for="item in navigationLinks" :key="item.label">
              <NuxtLink
                ...
                :class="
                  active === item.label ? 'text-primary' : 'text-black-alt hover:text-primary'
                "
              >
                {{ item.label }}
              </NuxtLink>
