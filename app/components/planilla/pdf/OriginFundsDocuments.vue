      ...
      return [
        {
          ...
          profession: getLabel(personal.value.profession, occupationsOptions) || '________________',
        },
      ]
    } else {
...
<template>
  <div class="spreadsheet" style="padding: 1rem">
    <div v-for="(doc, index) in documentsList" :key="index" class="page-break-container">
      ...
      <div class="spreadsheet__body mt-12 px-4">
        ...
        <div v-if="doc.type === 'NATURAL'" class="space-y-6 text-justify text-sm leading-loose">
          <p>
            Quien suscribe
            <span class="border-black border-b px-1 font-bold">{{ doc.name }}</span
            >, venezolano(a), mayor de edad, domiciliado(a) en
            <span class="border-black border-b px-1 font-bold">{{ doc.address }}</span>
            y titular de la Cédula de Identidad N°
            <span class="border-black border-b px-1 font-bold">{{ doc.id }}</span
            >, Profesión
            <span class="border-black border-b px-1 font-bold">{{ doc.profession }}</span
            >, por la presente declaro bajo fe de juramento que el origen de los fondos, son de mi
            peculio y de procedencia de actividades lícitas.
          </p>
          <p>
            Igualmente, declaro bajo fe de juramento no estar relacionado; ni pretender involucrar a
            <span class="font-bold">Maximiza Casa de Bolsa, C.A</span> en forma alguna, con las
            actividades ilícitas que puedan servir de instrumento para cometer el delito de
            legitimación de capitales, financiamiento al terrorismo, Financiamiento a la
        ...
        <div v-else class="space-y-6 text-justify text-sm leading-loose">
          <p>
            Quien suscribe
            <span class="border-black border-b px-1 font-bold">{{ doc.repName }}</span
            >, venezolano(a), mayor de edad, domiciliado(a) en
            <span class="border-black border-b px-1 font-bold">{{ doc.repAddress }}</span>
            y titular de la Cédula de Identidad N°
            <span class="border-black border-b px-1 font-bold">{{ doc.repId }}</span
            >, actuando en mi carácter de accionista / representante de la sociedad mercantil:
            <span class="border-black border-b px-1 font-bold">{{ doc.companyName }}</span
            >, por la presente declaro bajo fe de juramento que el origen de los fondos de mi
            representada, son de su peculio y de procedencia de actividades lícitas.
          </p>
          <p>
            Igualmente, declaro bajo fe de juramento no estar relacionado; ni pretender involucrar a
            <span class="font-bold">Maximiza Casa de Bolsa, C.A</span> en forma alguna, con las
        ...
        <p class="mt-8 text-justify text-sm leading-loose">
          ...
          <span class="border-black border-b px-2 font-bold">{{ dateParts.day }}</span>
          días del mes de
          <span class="border-black border-b px-2 font-bold">{{ dateParts.month }}</span>
          de 20<span class="border-black border-b px-1 font-bold">{{ dateParts.year }}</span
          >.
        </p>
