    ...
    <div v-for="(doc, index) in documentsList" :key="index" class="page-break-container">
      ...
      <div class="spreadsheet__body mt-12 px-4">
        ...
        <div v-if="doc.type === 'NATURAL'" class="space-y-6 text-justify text-sm leading-loose">
          <p>
            Quien suscribe
            <span class="border-b border-black px-1 font-bold">{{ doc.name }}</span
            >, venezolano(a), mayor de edad, domiciliado(a) en
            <span class="border-b border-black px-1 font-bold">{{ doc.address }}</span>
            y titular de la Cédula de Identidad N°
            <span class="border-b border-black px-1 font-bold">{{ doc.id }}</span
            >, Profesión
            <span class="border-b border-black px-1 font-bold">{{ doc.profession }}</span
        ...
        <div v-else class="space-y-6 text-justify text-sm leading-loose">
          <p>
            Quien suscribe
            <span class="border-b border-black px-1 font-bold">{{ doc.repName }}</span
            >, venezolano(a), mayor de edad, domiciliado(a) en
            <span class="border-b border-black px-1 font-bold">{{ doc.repAddress }}</span>
            y titular de la Cédula de Identidad N°
            <span class="border-b border-black px-1 font-bold">{{ doc.repId }}</span
            >, actuando en mi carácter de accionista / representante de la sociedad mercantil:
            <span class="border-b border-black px-1 font-bold">{{ doc.companyName }}</span
        ...
        <p class="mt-8 text-justify text-sm leading-loose">
          ...
          <span class="border-b border-black px-2 font-bold">{{ dateParts.day }}</span>
          días del mes de
          <span class="border-b border-black px-2 font-bold">{{ dateParts.month }}</span>
          de 20<span class="border-b border-black px-1 font-bold">{{ dateParts.year }}</span
          >.
        </p>
