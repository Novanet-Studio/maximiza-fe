  ...
  <form class="form-section" @submit.prevent>
    <div v-if="type === 'persona-natural'" class="form-section">
      ...
      <FormBaseLayout :style="'lg:grid-cols-2'">
        <FormBaseSelect name="economicActivity" label="Actividad económica" class="col-span-full"
          v-model="economicActivity" :options="economicActivityOptions" :error-message="errors.economicActivity"
          required />

        <FormBaseSelect name="specificActivity" label="Actividad específica" class="col-span-full"
    ...
    <div v-if="type === 'persona-natural'" class="form-section">
      ...
      <FormBaseLayout :style="'xl:grid-cols-4'">
        <FormBaseCheckbox name="source-dependencia" v-model="incomeSource" value="dependencia"
          label="Relación de dependencia" />

        <FormBaseCheckbox name="source-propio" v-model="incomeSource" value="propio" label="Negocio propio" />

        <FormBaseCheckbox name="source-otros" v-model="incomeSource" value="otros" label="Otros ingresos" />
      </FormBaseLayout>
      ...
      <div class="form-section">
        <div v-if="incomeSource.includes('dependencia')" class="form-subsection">
          <h5 class="form-subsection-title">
            Relación de dependencia
          </h5>
          <FormBaseLayout>
            <FormBaseInput name="company.name" label="Nombre de la empresa" v-model="companyName"
              :error-message="errors['company.name']" required />

            <div class="w-full">
              <FormBaseLabel htmlFor="company.rifNumber" label="RIF" required />
              <div class="flex gap-1">
                <div class="w-16 shrink-0">
                  <FormBaseSelect name="company.rifType" v-model="companyRifType" :options="docTypeOptionsAlt"
                    :error-message="errors['company.rifType'] ? ' ' : ''" />
                </div>
                <div class="w-full">
                  <FormBaseInput name="company.rifNumber" v-model="companyRifNumber" placeholder="12345678" type="tel"
                    :error-message="errors['company.rifNumber']" />
                </div>
              </div>
              <FormError v-if="!errors['company.rifNumber'] && errors['company.rifType']" error="Seleccione tipo" />
            </div>

            <FormBaseInput name="company.rol" label="Cargo dentro de la empresa" v-model="companyRol"
              :error-message="errors['company.rol']" required />

            <FormBaseSelect name="company.branch" label="Ramo de la empresa" v-model="companyBranch"
              :options="economicActivityOptions" :error-message="errors['company.branch']" class="col-span-full"
              required />

            <FormBaseInput label="Remuneración" subLabelType="BS" name="company.remuneration"
              v-model="companyRemuneration" type="number" :error-message="errors['company.remuneration'] ? ' ' : ''"
              :comment="'INCOMES'" />

            <FormBaseInput name="company.address" label="Dirección" v-model="companyAddress" />

            <FormBaseInput name="company.phone" label="Teléfono" v-model="companyPhone" />
          </FormBaseLayout>
        </div>
        ...
        <div v-if="incomeSource.includes('propio')" class="form-subsection">
          ...
          <FormBaseLayout>
            <FormBaseInput name="business.name" label="Nombre / Razón social" v-model="businessName"
              :error-message="errors['business.name']" required />

            <div class="w-full">
              <FormBaseLabel htmlFor="business.rifNumber" label="RIF" required />
              <div class="flex gap-1">
                <div class="w-16 shrink-0">
                  <FormBaseSelect name="business.rifType" v-model="businessRifType" :options="docTypeOptions"
                    :error-message="errors['business.rifType'] ? ' ' : ''" />
                </div>
                <div class="w-full">
                  <FormBaseInput name="business.rifNumber" v-model="businessRifNumber" placeholder="12345678" type="tel"
            ...
            <FormBaseInput name="business.constitutionDate" label="Fecha de constitución"
              v-model="businessConstitutionDate" type="date" :error-message="errors['business.constitutionDate']"
              required />

            <FormBaseInput name="business.registerData" label="Datos de registro" v-model="businessRegisterData"
              :error-message="errors['business.registerData']" required />

            <FormBaseInput name="business.fiscalAddress" label="Dirección fiscal" v-model="businessFiscalAddress"
              :comment="'RIF_REQUIRED'" />

            <FormBaseInput name="business.phone" label="Teléfono" v-model="businessPhone" />

            <FormBaseSelect name="business.branch" label="Ramo del negocio" v-model="businessBranch"
              :options="economicActivityOptions" :error-message="errors['business.branch']" class="col-span-full"
              required />

            <FormBaseInput label="Ingresos mensuales" subLabelType="BS" name="business.income" v-model="businessIncome"
              type="number" :comment="'INCOMES'" :error-message="errors['business.income'] ? ' ' : ''" />
          </FormBaseLayout>
          ...
          <div class="form-subsection --alt">
            ...
            <div v-for="(field, i) in businessProviderFields" :key="field.key">
              <FormBaseLayout use-buttons>
                <FormBaseInput :name="`business.providers[${i}].name`" label="Razón social" v-model="field.value.name"
                  :error-message="errors[`business.providers[${i}].name`]" />
                <FormBaseInput :name="`business.providers[${i}].location`" label="Ubicación"
                  v-model="field.value.location" :error-message="errors[`business.providers[${i}].location`]" />
                <template #button-bar>
                  <UiButton v-if="businessProviderFields.length > 1" :text="''" :icon="'close'" size="icon"
                    variant="danger" :onClick="() => removeBusinessProvider(i)" />
                </template>
              </FormBaseLayout>
            </div>

            <span class="self-end">
              <UiButton v-if="businessProviderFields.length < 3" suffix-icon text="Agregar proveedor" :icon="'plus'"
                size="sm" :onClick="handleAddBusinessProvider" />
            </span>
          </div>
          ...
          <div class="form-subsection --alt flex flex-col">
            ...
            <div v-for="(field, i) in businessClientFields" :key="field.key">
              <FormBaseLayout use-buttons>
                <FormBaseInput :name="`business.clients[${i}].name`" label="Razón social" v-model="field.value.name"
                  :error-message="errors[`business.clients[${i}].name`]" />
                <FormBaseInput :name="`business.clients[${i}].location`" label="Ubicación"
                  v-model="field.value.location" :error-message="errors[`business.clients[${i}].location`]" />
                <template #button-bar>
                  <UiButton v-if="businessClientFields.length > 1" :text="''" :icon="'close'" size="icon"
                    variant="danger" :onClick="() => removeBusinessClient(i)" />
                </template>
              </FormBaseLayout>
            </div>
            <span class="self-end">
              <UiButton v-if="businessClientFields.length < 3" suffix-icon text="Agregar cliente" :icon="'plus'"
                size="sm" :onClick="handleAddBusinessClient" />
            </span>
          </div>
        </div>
        ...
        <div v-if="incomeSource.includes('otros')" class="form-subsection">
          ...
          <FormBaseLayout>
            <FormBaseSelect class="col-span-full" name="otherIncomeSource" label="Actividad generadora"
              v-model="otherIncomeSource" :options="economicActivityOptions" :error-message="errors.otherIncomeSource"
              required />

            <FormBaseInput label="Ingresos mensuales" subLabelType="BS" name="otherIncomeAmount"
              v-model="otherIncomeAmount" type="number" :error-message="errors.otherIncomeAmount ? ' ' : ''"
              :comment="'INCOMES'" />
          </FormBaseLayout>
        </div>
      </div>
    ...
    <div v-if="type === 'persona-juridica'" class="form-section">
      ...
      <FormBaseLayout class="grid-cols-1 md:grid-cols-3">
        <FormBaseInput name="monthlyIncome" label="Ingresos" subLabelType="BS" v-model="monthlyIncome" type="number"
          :error-message="errors.monthlyIncome" />
        <FormBaseInput name="monthlySales" label="Ventas" subLabelType="BS" v-model="monthlySales" type="number"
          :error-message="errors.monthlySales" />
        <FormBaseInput name="monthlyExpenses" label="Egresos" subLabelType="BS" v-model="monthlyExpenses" type="number"
      ...
      <FormBaseLayout>
        <FormBaseInput name="islrYear" label="Año" v-model="islrYear" :error-message="errors.islrYear" type="number"
          :min="1900" :max="new Date().getFullYear()" required />

        <FormBaseInput name="islrAmount" label="Monto" subLabelType="BS" v-model="islrAmount"
      ...
      <div class="form-subsection --alt flex flex-col">
        ...
        <div class="w-full">
          <div v-for="(field, i) in stockholderFields" :key="field.key"
            class="mb-6 pb-6 border-b border-white-alt2 last:border-0 last:pb-0 last:mb-0">
            <FormBaseLayout use-buttons>
              <FormBaseInput :name="`stockholders[${i}].name`" label="Nombre completo" v-model="field.value.name"
                :error-message="errors[`stockholders[${i}].name`]" required />

              <div class="w-full">
                <FormBaseLabel :htmlFor="`stockholders[${i}].dniNumber`" label="Documento de identidad" required />
                <div class="flex gap-1">
                  <div class="w-16 shrink-0">
                    <FormBaseSelect :name="`stockholders[${i}].dniType`" v-model="field.value.dniType"
                      :options="docTypeOptions" :error-message="errors[`stockholders[${i}].dniType`]" required />
                  </div>
                  <div class="w-full">
                    <FormBaseInput :name="`stockholders[${i}].dniNumber`" v-model="field.value.dniNumber" type="tel"
                      :error-message="errors[`stockholders[${i}].dniNumber`]" required />
                  </div>
                </div>
              </div>

              <FormBaseInput :name="`stockholders[${i}].percentage`" label="% Accionario"
                v-model="field.value.percentage" type="number" :max="100" :min="0"
                :error-message="errors[`stockholders[${i}].percentage`]" />

              <FormBaseInput :name="`stockholders[${i}].cargo`" label="Cargo" v-model="field.value.cargo"
                :error-message="errors[`stockholders[${i}].cargo`]" />

              <FormBaseSelect :name="`stockholders[${i}].nationality`" v-model="field.value.nationality"
                label="Nacionalidad" :options="nationalityOptions"
                :error-message="errors[`stockholders[${i}].nationality`]" required />

              <FormBaseInput :name="`stockholders[${i}].address`" label="Direccion" v-model="field.value.address"
                :error-message="errors[`stockholders[${i}].address`]" />

              <div class="col-span-full space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <FormBaseSelect :name="`stockholders[${i}].esPep`" label="¿Es Persona Expuesta Políticamente (PEP)?"
                    v-model="field.value.esPep" :options="isPepOptions"
                    :error-message="errors[`stockholders[${i}].esPep`]" />

                  <FormBaseSelect :name="`stockholders[${i}].relatedWithPep`" label="¿Relacionado con PEP?"
                    v-model="field.value.relatedWithPep" :error-message="errors[`stockholders[${i}].relatedWithPep`]"
                    :options="relatedWithPepOptions" />
                </div>

                <div v-if="field.value.esPep === 'SI'">
                  <p class="font-black text-primary mb-4">
                    Detalle PEP
                  </p>
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <FormBaseInput :name="`stockholders[${i}].entityName`" label="Nombre del ente adscripción"
                      v-model="field.value.entityName" :error-message="errors[`stockholders[${i}].entityName`]" />

                    <FormBaseInput :name="`stockholders[${i}].position`" label="Cargo que desempeña"
                      v-model="field.value.position" :error-message="errors[`stockholders[${i}].position`]" />

                    <FormBaseSelect :name="`stockholders[${i}].country`" label="País" v-model="field.value.country"
                      :options="countriesOptions" :error-message="errors[`stockholders[${i}].country`]" />

                    <FormBaseInput :name="`stockholders[${i}].relatedIdentification`"
                      label="Identificación del relacionado (En caso de aplicar)"
                      v-model="field.value.relatedIdentification" :error-message="errors[`stockholders[${i}].relatedIdentification`]
                        " />
                  </div>
                </div>
              </div>
              ...
              <template #button-bar>
                <UiButton v-if="stockholderFields.length > 1" :text="''" :icon="'close'" size="icon" variant="danger"
        ...
        <span class="self-end">
          <UiButton v-if="stockholderFields.length < 3" suffix-icon text="Agregar accionista" :icon="'plus'" size="sm"
      ...
      <div class="form-subsection --alt flex flex-col">
        ...
        <div class="w-full">
          <div v-for="(field, i) in legalFields" :key="field.key">
            <FormBaseLayout use-buttons>
              <FormBaseInput :name="`legalRepresentatives[${i}].name`" label="Nombre" v-model="field.value.name"
                :error-message="errors[`legalRepresentatives[${i}].name`]" />

              <div class="w-full">
                <FormBaseLabel :htmlFor="`legalRepresentatives[${i}].dniNumber`" label="Documento de identidad"
                  required />
                <div class="flex gap-1">
                  <div class="w-16 shrink-0">
                    <FormBaseSelect :name="`legalRepresentatives[${i}].dniType`" v-model="field.value.dniType"
                      :options="docTypeOptions" :error-message="errors[`legalRepresentatives[${i}].dniType`]
                        " />
                  </div>
                  <div class="w-full">
                    <FormBaseInput :name="`legalRepresentatives[${i}].dniNumber`" v-model="field.value.dniNumber"
                      type="tel" :error-message="errors[`legalRepresentatives[${i}].dniNumber`]
                        " />
                  </div>
                </div>
              </div>

              <FormBaseInput :name="`legalRepresentatives[${i}].cargo`" label="Cargo" v-model="field.value.cargo"
                :error-message="errors[`legalRepresentatives[${i}].cargo`]" />

              <FormBaseSelect :name="`legalRepresentatives[${i}].condition`" label="Condición"
                v-model="field.value.condition" :options="conditionOptions"
                :error-message="errors[`legalRepresentatives[${i}].condition`]" />

              <FormBaseSelect :name="`legalRepresentatives[${i}].nationality`" v-model="field.value.nationality"
                label="Nacionalidad" :options="nationalityOptions" :error-message="errors[`legalRepresentatives[${i}].nationality`]
                  " required />

              <FormBaseInput :name="`legalRepresentatives[${i}].address`" label="Direccion"
                v-model="field.value.address" :error-message="errors[`legalRepresentatives[${i}].address`]" />

              <div class="col-span-full space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <FormBaseSelect :name="`legalRepresentatives[${i}].esPep`"
                    label="¿Es Persona Expuesta Políticamente (PEP)?" v-model="field.value.esPep"
                    :options="isPepOptions" :error-message="errors[`legalRepresentatives[${i}].esPep`]" />

                  <FormBaseSelect :name="`legalRepresentatives[${i}].relatedWithPep`" label="¿Relacionado con PEP?"
                    v-model="field.value.relatedWithPep" :error-message="errors[`legalRepresentatives[${i}].relatedWithPep`]
                      " :options="relatedWithPepOptions" />
                </div>
              </div>

              <template #button-bar>
                <UiButton v-if="legalFields.length > 1" :text="''" :icon="'close'" size="icon" variant="danger"
        ...
        <span class="self-end">
          <UiButton v-if="legalFields.length < 3" suffix-icon text="Agregar" :icon="'plus'" size="sm"
        ...
        <div class="w-full">
          <div v-for="(field, i) in juridicaProviderFields" :key="field.key">
            <FormBaseLayout use-buttons>
              ...
              <FormBaseInput :name="`providers[${i}].location`" label="Ubicación" v-model="field.value.location"
        ...
        <div class="w-full">
          <div v-for="(field, i) in juridicaClientFields" :key="field.key">
            <FormBaseLayout use-buttons>
              ...
              <FormBaseInput :name="`clients[${i}].location`" label="Ubicación" v-model="field.value.location"
      ...
      <div class="form-subsection --alt flex flex-col">
        ...
        <div class="w-full">
          <div v-for="(field, i) in companyFields" :key="field.key">
            <FormBaseLayout use-buttons>
              <FormBaseInput :name="`relatedCompanies[${i}].name`" label="Razón social" v-model="field.value.name"
                :error-message="errors[`relatedCompanies[${i}].name`]" />
              <FormBaseInput :name="`relatedCompanies[${i}].activity`" label="Actividad económica"
                v-model="field.value.activity" :error-message="errors[`relatedCompanies[${i}].activity`]" />

              <div class="w-full">
                <FormBaseLabel :htmlFor="`relatedCompanies[${i}].rifNumber`" label="RIF" required />
                <div class="flex gap-1">
                  <div class="w-16 shrink-0">
                    <FormBaseSelect :name="`relatedCompanies[${i}].rifType`" v-model="field.value.rifType"
                      :error-message="errors[`relatedCompanies[${i}].rifType`]" :options="docTypeOptions" />
                  </div>
                  <div class="w-full">
                    <FormBaseInput :name="`relatedCompanies[${i}].rifNumber`" v-model="field.value.rifNumber"
              ...
              <template #button-bar>
                <UiButton v-if="companyFields.length > 1" :text="''" :icon="'close'" size="icon" variant="danger"
        ...
        <span class="self-end">
          <UiButton v-if="companyFields.length < 3" suffix-icon text="Agregar" :icon="'plus'" size="sm"
    ...
    <div class="form-section">
      ...
      <FormBaseLayout>
        ...
        <FormBaseSelect name="bankReference.productType" label="Tipo de cuenta"
          ...
          :error-message="errors['bankReference.productType']" required />

        <FormBaseInput name="bankReference.accountNumber" label="Número de cuenta (20 dígitos)"
          v-model="bankAccountNumber" :maxlength="20" placeholder="12345..."
          :error-message="errors['bankReference.accountNumber']" required />

        <FormBaseInput name="bankReference.averageAmount" type="number" label="Cifras promedio mensuales"
          subLabelType="BS" v-model="bankAverageAmount" :error-message="errors['bankReference.averageAmount']"
          required />
      </FormBaseLayout>
    </div>
  </form>
