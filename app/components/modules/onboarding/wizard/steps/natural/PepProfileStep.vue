...
const pepStatusOptions = [
  { value: "isPep", label: "Soy una PEP (PEP)" },
...
<template>
  <form class="flex flex-col gap-4" @submit.prevent>
    <FormTitle text="Persona expuesta políticamente (PEP)"
      description="<strong>Definición PEP:</strong> Persona natural que desempeña o ha desempeñado funciones públicas destacadas en el país o en el extranjero." />

    <FormBaseRadio name="pepStatus" v-model="pepStatus" :options="pepStatusOptions" :error-message="errors.pepStatus" />

    <transition enter-active-class="transition-all duration-500 ease-out"
      enter-from-class="opacity-0 max-h-0 overflow-hidden" enter-to-class="opacity-100 max-h-[1000px] overflow-hidden"
      leave-active-class="transition-all duration-300 ease-in"
      leave-from-class="opacity-100 max-h-[1000px] overflow-hidden" leave-to-class="opacity-0 max-h-0 overflow-hidden">
      <div v-if="showDetails" class="flex flex-col gap-4">
        <FormBaseDivider />

        <FormTitle text="Detalles de la vinculación" />

        <FormBaseLayout :style="'xl:grid-cols-2'">
          <FormBaseInput name="entityName" label="Nombre del ente de adscripción" v-model="entityName"
            :error-message="errors.entityName" required />
          <FormBaseInput name="position" label="Cargo que desempeña" v-model="position" :error-message="errors.position"
            required />
        </FormBaseLayout>

        <FormBaseLayout :style="'xl:grid-cols-2'">
          <FormBaseSelect name="country" label="País" v-model="country" :options="countriesOptions"
            :error-message="errors.country" required />
          <FormBaseInput name="relatedIdentification" label="Identificación del relacionado (En caso de aplicar)"
            v-model="relatedIdentification" placeholder="Documento ID" />
        </FormBaseLayout>
      </div>
    </transition>
  </form>
