<script setup lang="ts">
  ...
  const schema = yup.object({
    ...
    phones: yup.string().required('Teléfono requerido').matches(PHONE_REGEX, 'Teléfono inválido'),
    ...
    legalRepresentativePhones: yup.string().when('hasLegalRepresentative', {
      is: 'SI',
      then: (schema) => schema.required('Requerido').matches(PHONE_REGEX, 'Teléfono inválido'),
      otherwise: (schema) => schema.notRequired(),
    }),
