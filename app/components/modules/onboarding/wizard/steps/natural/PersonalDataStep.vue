...
<template>
  <form class="form-section" @submit.prevent>
    ...
    <FormBaseLayout>
      <FormBaseInput name="firstName" label="Nombres" v-model="firstName" :error-message="errors.firstName" required />
      <FormBaseInput name="lastName" label="Apellidos" v-model="lastName" :error-message="errors.lastName" required />
      ...
      <div class="w-full">
        ...
        <div class="flex gap-1">
          <div class="w-16 shrink-0">
            <FormBaseSelect name="identificationType" v-model="identificationType" :options="docTypeOptions"
              :error-message="errors.identificationType ? ' ' : ''" />
          </div>

          <div class="w-full">
            <FormBaseInput name="identificationNumber" v-model="identificationNumber" placeholder="12345678" type="tel"
    ...
    <FormBaseLayout :style="'xl:grid-cols-4'">
      <FormBaseInput name="birthDate" label="Fecha de Nacimiento" type="date" v-model="birthDate" :max="minAgeDate(18)"
        :error-message="errors.birthDate" required />

      <FormBaseSelect name="nationality" label="Nacionalidad" v-model="nationality" :options="nationalityOptions"
        :error-message="errors.nationality" required />

      <FormBaseSelect name="birthPlace" label="Lugar de nacimiento" v-model="birthPlace"
        :error-message="errors.birthPlace" :options="countriesOptions" required />

      <FormBaseInput name="otherNationality" label="Otra nacionalidad" subLabelType="APPLY"
        v-model="otherNationality" />
    </FormBaseLayout>

    <FormBaseLayout :style="'xl:grid-cols-4'">
      <FormBaseSelect name="maritalStatus" label="Estado civil" v-model="maritalStatus" :options="maritalStatusOptions"
        :error-message="errors.maritalStatus" required />

      <FormBaseSelect name="gender" label="Género" v-model="gender" :options="genderOptions"
        :error-message="errors.gender" required />
    </FormBaseLayout>

    <FormBaseDivider />

    <FormTitle text="Información de contacto" />

    <FormBaseLayout :style="'xl:grid-cols-4'">
      <FormBaseInput name="address" label="Dirección de domicilio" v-model="address" placeholder="Dirección completa"
        :error-message="errors.address" required class="col-span-2" :comment="'RIF_REQUIRED'" />

      <FormBaseInput name="phones" label="Teléfono" type="tel" v-model="phones" :error-message="errors.phones"
        required />

      <FormBaseInput name="email" label="Correo electrónico" type="email" v-model="email" :error-message="errors.email"
        required />
    </FormBaseLayout>

    <FormBaseDivider />

    <FormTitle text="Datos de ingresos" />

    <FormBaseLayout>
      <FormBaseSelect class="col-span-full" name="profession" label="Profesión u oficio" v-model="profession"
        :options="occupationsOptions" :error-message="errors.profession" required />

      <FormBaseSelect class="col-span-full" name="incomeSource" label="Fuente de ingreso" v-model="incomeSource"
        :options="economicActivityOptions" :error-message="errors.incomeSource" required />
    </FormBaseLayout>

    <FormBaseDivider />

    <FormTitle text="Datos personales del cónyuge (En caso de aplicar)" />

    <FormBaseLayout>
      <FormBaseInput name="spouseName" label="Nombre completo del cónyuge" v-model="spouseName" />
      <FormBaseInput name="spouseIdentification" label="Documento identidad cónyuge" v-model="spouseIdentification" />
    </FormBaseLayout>
    ...
    <FormBaseLayout>
      <FormBaseRadio name="hasLegalRepresentative" label="¿Posee representante legal, apoderado y/o autorizado?"
        v-model="hasLegalRepresentative" :options="booleanOptions" :error-message="errors.hasLegalRepresentative" />
    </FormBaseLayout>
    ...
    <transition enter-active-class="transition duration-200 ease-out" enter-from-class="transform scale-95 opacity-0"
      ...
      <div v-if="hasLegalRepresentative === 'SI'">
        <FormBaseLayout>
          <FormBaseInput name="legalRepresentativeFullname" label="Nombre completo del representante"
            v-model="legalRepresentativeFullname" :error-message="errors.legalRepresentativeFullname" required />

          <div class="w-full">
            <FormBaseLabel htmlFor="legalRepresentativeIdentificationNumber" label="Documento de identidad / RIF"
              required />

            <div class="flex gap-1">
              <div class="w-16 shrink-0">
                <FormBaseSelect name="legalRepresentativeIdentificationType"
                  v-model="legalRepresentativeIdentificationType" :options="docTypeOptions" :error-message="errors.legalRepresentativeIdentificationType ? ' ' : ''
                    " />
              </div>

              <div class="w-full">
                <FormBaseInput name="legalRepresentativeIdentificationNumber"
                  v-model="legalRepresentativeIdentificationNumber" placeholder="12345678" type="tel" :error-message="errors.legalRepresentativeIdentificationNumber
                    " :comment="'RIF_REQUIRED'" />
              </div>
            </div>
          </div>

          <FormBaseSelect name="legalRepresentativeBirthPlace" label="Lugar de nacimiento"
            v-model="legalRepresentativeBirthPlace" :options="countriesOptions"
            :error-message="errors.legalRepresentativeBirthPlace" required />

          <FormBaseInput name="legalRepresentativeBirthDate" label="Fecha de nacimiento" type="date"
            v-model="legalRepresentativeBirthDate" :max="minAgeDate(18)"
            :error-message="errors.legalRepresentativeBirthDate" required />

          <FormBaseInput name="legalRepresentativePhones" label="Teléfono" type="tel"
            v-model="legalRepresentativePhones" :error-message="errors.legalRepresentativePhones" required />

          <FormBaseInput name="legalRepresentativeDocumentData" label="Datos del documento"
            v-model="legalRepresentativeDocumentData" :error-message="errors.legalRepresentativeDocumentData"
            required />
        </FormBaseLayout>
      </div>
    </transition>
