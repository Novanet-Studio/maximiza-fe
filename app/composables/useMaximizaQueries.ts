...
export const useMaximizaQueries = () => {
  ...
  const strapiUrl = config.public.strapi.url as string;

  const graphql = <T>(query: string, variables?: Record<string, any>): Promise<{ data: T }> =>
    $fetch<{ data: T }>(`${strapiUrl}/graphql`, {
      method: "POST",
