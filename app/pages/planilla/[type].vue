<script setup lang="ts">
  ...
  const activeComponent = computed(() => stepsComponents[type][wizard.state.value.currentStep])
  ...
  const pageTitle = computed(() => (type === 'juridica' ? 'Persona jurídica' : 'Persona natural'))
</script>

<template>
  <div class="bg-white w-full pb-24">
    ...
    <section class="mx-auto mt-16">
      <div class="rounded-sm border border-maximiza-gris5 bg-maximiza-blanco2 p-4 shadow-sm md:p-8">
        <div>
          <KeepAlive>
        ...
        <div class="mt-12 flex justify-between border-t border-maximiza-gris5 pt-4">
          <button
            @click="wizard.prevStep"
            class="button-secondary --medium"
            :disabled="wizard.state.value.currentStep === 0"
            :class="{
              'cursor-not-allowed opacity-50': wizard.state.value.currentStep === 0,
            }"
          >
            Atrás
          </button>
